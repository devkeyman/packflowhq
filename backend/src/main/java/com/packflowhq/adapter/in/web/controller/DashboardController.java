...
public class DashboardController {
    ...
    public ResponseEntity<ApiResponse<DashboardSummary>> getSummary() {
        ...
        Map<com.packflowhq.domain.model.Priority, Long> priorityCounts = activeWorkOrders.stream()
                                                                                         .collect(Collectors.groupingBy(WorkOrderModel::getPriority, Collectors.counting()));

        summary.setUrgentCount(priorityCounts.getOrDefault(com.packflowhq.domain.model.Priority.URGENT, 0L).intValue());
        summary.setHighPriorityCount(priorityCounts.getOrDefault(com.packflowhq.domain.model.Priority.HIGH, 0L).intValue());
